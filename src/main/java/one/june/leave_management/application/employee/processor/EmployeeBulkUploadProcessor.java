package one.june.leave_management.application.employee.processor;
...
/**
 * Processes bulk upload jobs for employees.
...
@Component
@Slf4j
public class EmployeeBulkUploadProcessor {
    ...
    private final EmployeeBulkUploadStrategy employeeBulkUploadStrategy;

    public EmployeeBulkUploadProcessor(EmployeeService employeeService,
                                       ...
                                       EmployeeBulkUploadStrategy employeeBulkUploadStrategy) {
        ...
        this.employeeBulkUploadStrategy = employeeBulkUploadStrategy;
    }
     ...
     * @param commands The list of employee creation commands
     * @param csvMetadata The list of CSV metadata maps (one per row)
     */
    public void processBulkUpload(UUID jobId, List<EmployeeCreateCommand> commands,
                                   List<Map<String, String>> csvMetadata) {
        log.info("Starting processing for employee bulk upload job {}", jobId);
        ...
        try {
            ...
            for (int i = 0; i < commands.size(); i++) {
                EmployeeCreateCommand command = commands.get(i);
            ...
            log.error("Failed to process employee bulk upload job {}", jobId, e);
    ...
    protected void processSingleRow(BulkUploadJob job, EmployeeCreateCommand command,
    ...
    protected void updateJobAsCompleted(BulkUploadJob job) {
        ...
        log.info("Completed employee bulk upload job {} - Success: {}, Failed: {}",
