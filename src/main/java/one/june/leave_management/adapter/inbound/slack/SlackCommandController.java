...
public class SlackCommandController {
    ...
    /**
     * Handles the /delete-leave slash command from Slack
     * <p>
     * Process:
     * <ol>
     *   <li>Read request body</li>
    ...
    @Auditable("Slack delete-leave command endpoint")
    public ResponseEntity<SlackCommandResponse> handleDeleteLeaveCommand(
            HttpServletRequest request,
            @RequestBody byte[] rawBody
    ) {
        String requestBody = new String(rawBody, StandardCharsets.UTF_8);
        log.info("Received Slack delete-leave command request");
        ...
        log.info("Parsed /delete-leave command from user: {} in channel: {}",
                slackRequest.getUserId(),
                slackRequest.getChannelName());

        // Step 3: Route to orchestrator for business logic
        slackLeaveOrchestrator.handleDeleteLeaveSlashCommand(slackRequest);
