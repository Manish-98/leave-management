...
class AuditIntegrationTest {
    ...
    private static final String AUDIT_USER_1_SLACK = "U301";
    private static final String AUDIT_USER_2_SLACK = "U302";
    private static final String AUDIT_USER_1_UUID = "123e4567-e89b-12d3-a456-426614174300";
    private static final String AUDIT_USER_2_UUID = "123e4567-e89b-12d3-a456-426614174301";
    ...
    void setUp() {
        ...
        // Delete leaves by employee UUID instead of Slack ID
        jdbcTemplate.update("DELETE FROM leave WHERE user_id IN (?, ?)", AUDIT_USER_1_UUID, AUDIT_USER_2_UUID);
        jdbcTemplate.update("DELETE FROM employee WHERE slack_id IN (?, ?)", AUDIT_USER_1_SLACK, AUDIT_USER_2_SLACK);

        jdbcTemplate.update(
                "INSERT INTO employee (id, name, slack_id, date_of_joining, active, region, created_at, updated_at) " +
                "VALUES (?, 'Audit User 1', ?, '2020-01-01', true, 'PUNE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                AUDIT_USER_1_UUID, AUDIT_USER_1_SLACK);

        jdbcTemplate.update(
                "INSERT INTO employee (id, name, slack_id, date_of_joining, active, region, created_at, updated_at) " +
                "VALUES (?, 'Audit User 2', ?, '2020-01-01', true, 'PUNE', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",
                AUDIT_USER_2_UUID, AUDIT_USER_2_SLACK);
    ...
    void auditLogShouldCaptureSuccessfulRequest() {
        ...
        assertThat(responseBody).contains("\"slackId\":\"" + AUDIT_USER_1_SLACK + "\"");
        ...
        // Verify request body was captured (contains userId - now employee UUID)
        String requestBody = (String) auditLog.get("request_body");
        assertThat(requestBody).isNotNull();
        assertThat(requestBody).contains(AUDIT_USER_1_UUID);

        // Verify response body was captured (contains userId - now employee UUID)
        String responseBodyDb = (String) auditLog.get("response_body");
        assertThat(responseBodyDb).isNotNull();
        assertThat(responseBodyDb).contains(AUDIT_USER_1_UUID);

        // Verify user ID was extracted (now employee UUID)
        assertThat(auditLog.get("user_id")).isEqualTo(AUDIT_USER_1_UUID);
    ...
    void auditLogShouldCaptureFailedRequest() {
        ...
        LeaveIngestionRequest request = LeaveIngestionRequest.builder()
                ...
                .userId("123e4567-e89b-12d3-a456-426614174301")
                .dateRange(DateRange.builder()
        ...
        if (auditLog != null) {
            ...
            String requestBody = (String) auditLog.get("request_body");
            assertThat(requestBody).isNotNull();
            assertThat(requestBody).contains(AUDIT_USER_2_UUID);
